...
class CNcN(nn.Module):
    def __init__(self, input_shape, start_filter, conv_kernel_size, activation, dr_rate, deconv_kernel_size, pres=False):
        super(CNcN, self).__init__()

        self.configs = configs
        self.pres = pres.upper()
        self.psu = False
        self.motion = False
        ...
        self.mp1 = self._make_maxp_layer(start_filter, 0.3)
        self.enc2 = self._make_enc_layer(start_filter, start_filter * 2, 1, conv_kernel_size, activation)
        self.mp2 = self._make_maxp_layer(start_filter * 2, 0.3)
        self.enc3 = self._make_enc_layer(start_filter * 2, start_filter * 4, 1 , conv_kernel_size, activation)
        self.mp3 = self._make_maxp_layer(start_filter * 4, 0.3)
        self.enc4 = self._make_enc_layer(start_filter * 4, start_filter * 8, 1, conv_kernel_size, activation)  # Depth becomes 1
        self.mp4 = self._make_maxp_layer(start_filter * 8, 0.2)

        # Decoding
        self.midnet= self._make_dec_layer(start_filter * 8, start_filter * 8, 1, deconv_kernel_size, activation, 0.2)
        self.crop3d = Cropping3D(cropping=((0, 0), (1, 0), (1, 0)))
        self.crop3d1 = Cropping3D(cropping=((2, 0), (0, 0), (0, 0)))
        self.dec1 = self._make_dec_layer(start_filter * 16, start_filter * 4, 1, deconv_kernel_size, activation, 0.2)
        self.crop3d2 = Cropping3D(cropping=((4, 0), (0, 0), (0, 0)))
        self.dec2 = self._make_dec_layer(start_filter * 8, start_filter * 2, 1, deconv_kernel_size, activation, 0.1)
        self.crop3d3 = Cropping3D(cropping=((6, 0), (0, 0), (0, 0)))
        self.dec3 = self._make_dec2_layer(start_filter * 4, start_filter, 1, deconv_kernel_size, activation, 0.1)
        self.crop3d4 = Cropping3D(cropping=((6, 0), (0, 0), (0, 0)))
        self.dec4 = self._make_dec3_layer(start_filter * 2, start_filter // 2, activation, 0.1)

        self.final_conv = nn.Conv3d(start_filter // 2, 1, kernel_size=(1, 1, 1), padding=(0, 0, 0))

        if self.pres == 'PSU':
            self.psu = PSU(640, 640)    # PSU(640, 640)
        elif self.pres == 'PSU_ONE':
            self.psu = PSU_one(640, 640)    # PSU_one(640, 640)
        elif self.pres == 'CMN':
            self.motion = CMN(self.configs["data"]["input_length"], self.configs["data"]["n_leadtimes"],
                              self.configs["model"]["motion_net_core"])
            ...
            self.motion = RMN(self.configs["data"]["input_length"], self.configs["data"]["n_leadtimes"],
                              self.configs["model"]["motion_net_core"])
            sample_tensor = torch.zeros(1, 1, self.configs["model"]["img_height"], self.configs["model"]["img_width"])
            self.grid = make_grid(sample_tensor)

    def forward(self, x):
        batch_size, input_channels, seq_length, height, width = x.shape
        ...
        cncn_result = self.final_conv(d4)


        if self.psu != False:
            # 调用光流法增加预感门
            cncn_result = self.psu(cncn_result, x)
        n_leadtimes = configs["data"]["n_leadtimes"]
        if self.motion != False:
            # 调用光流法增加预感门
            input_frames = x.reshape(batch_size, seq_length, height, width)
            motion = self.motion(input_frames)
            motion_ = motion.reshape(batch_size, n_leadtimes, 2, height, width)
            series = []
            last_frames = x[:, 0, (seq_length - 1):seq_length, :, :]
            ...
            for i in range(n_leadtimes):
                ...
                intensity_ = cncn_result[:, :, i]
                last_frames = last_frames + intensity_
                series.append(last_frames)
            cncn_result = torch.cat(series, dim=1)

            cncn_result = cncn_result.reshape(batch_size, 1, n_leadtimes, height, width)
        # print("cncn_result最大值：%f" % max_value)
        # print("cncn_result均值：%f" % mean_value)
        return cncn_result

    def _make_enc_layer(self, in_channels, out_channels, depth, conv_kernel_size, activation):
        return nn.Sequential(
            nn.Conv3d(in_channels, out_channels, kernel_size=(conv_kernel_size, conv_kernel_size, conv_kernel_size), padding=(1, 1, 1)),
    ...
    def _make_dec_layer(self, in_channels, out_channels, depth, deconv_kernel_size, activation, dr_rate):
        return nn.Sequential(
            ...
            nn.ConvTranspose3d(out_channels, out_channels, kernel_size=(depth, deconv_kernel_size, deconv_kernel_size), stride=(1, 2, 2),padding=(0,0,0)),
            nn.ReLU() if activation == 'relu' else nn.Sigmoid(),
            nn.BatchNorm3d(out_channels),
            nn.Dropout3d(dr_rate)
        )
...
class RNcN(nn.Module):
    def __init__(self, convlstm_input_channels, convlstm_hidden_channels, convlstm_kernel_size,
        ...
        if self.pres == 'PSU':
            self.psu = PSU(640, 640)    # 640, 640
        elif self.pres == 'PSU_ONE':
            self.psu = PSU_one(640, 640)    # 640, 640
        elif self.pres == 'CMN':
            self.motion = CMN(self.configs["data"]["input_length"], self.configs["data"]["n_leadtimes"],
                              self.configs["model"]["motion_net_core"])
            sample_tensor = torch.zeros(1, 1, self.configs["model"]["img_height"], self.configs["model"]["img_width"])
    ...
    def forward(self, input):
        ...
        # print("rncn-in最大值：：%f" % in_max)
        # print("rncn-in均值：：%f" % in_mean)
        # unit1
        ...
        rncn_result = self.conv3d7(usample3)

        if self.psu != False:
            # 调用光流法增加预感门
            rncn_result = self.psu(rncn_result, input)
        n_leadtimes = configs["data"]["n_leadtimes"]
        if self.motion != False:
            # 调用光流法增加预感门
            ...
            for i in range(n_leadtimes):
                last_frames = warp(last_frames, motion_[:, i], grid.cuda(), mode="nearest", padding_mode="border")
                intensity_ = rncn_result[:, :, i]
                last_frames = last_frames + intensity_
                series.append(last_frames)
            rncn_result = torch.cat(series, dim=1)

            rncn_result = rncn_result.reshape(batch_size, 1, n_leadtimes, height, width)

        return rncn_result


class Evolution_Network(nn.Module):
    def __init__(self, n_channels, n_classes, base_c=8, bilinear=True):
        ...
        self.down1 = CDown(base_c * 1, base_c * 2)
        self.down2 = CDown(base_c * 2, base_c * 4)
        self.down3 = CDown(base_c * 4, base_c * 8)
        factor = 2 if bilinear else 1
        self.down4 = CDown(base_c * 8, base_c * 16 // factor)

        self.up1 = CUp(base_c * 16, base_c * 8 // factor, bilinear)
        self.up2 = CUp(base_c * 8, base_c * 4 // factor, bilinear)
        self.up3 = CUp(base_c * 4, base_c * 2 // factor, bilinear)
        self.up4 = CUp(base_c * 2, base_c * 1, bilinear)
        self.outc = OutConv(base_c * 1, n_classes)
        self.gamma = nn.Parameter(torch.zeros(1, n_classes, 1, 1), requires_grad=True)

        self.up1_v = CUp(base_c * 16, base_c * 8 // factor, bilinear)
        self.up2_v = CUp(base_c * 8, base_c * 4 // factor, bilinear)
        self.up3_v = CUp(base_c * 4, base_c * 2 // factor, bilinear)
        self.up4_v = CUp(base_c * 2, base_c * 1, bilinear)
...
class RMN(nn.Module):
    """
       recurrent motion net
    """

    def __init__(self, n_channels, n_classes, base_c=64, bilinear=True, convlstm_kernel_size=3):
        super(RMN, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear
        self.base_c = base_c
        self.convlstm = LSTMConv(n_channels, base_c, convlstm_kernel_size)
        self.down1 = RDown(base_c * 1, base_c * 2)
        self.down2 = RDown(base_c * 2, base_c * 4)
        self.down3 = RDown(base_c * 4, base_c * 8)
        factor = 2 if bilinear else 1
        self.down4 = RDown(base_c * 8, base_c * 16 // factor)

        self.up1_v = RUp(base_c * 16, base_c * 8 // factor, bilinear)
        self.up2_v = RUp(base_c * 8, base_c * 4 // factor, bilinear)
        self.up3_v = RUp(base_c * 4, base_c * 2 // factor, bilinear)
        self.up4_v = RUp(base_c * 2, base_c * 1, bilinear)
        self.outc_v = OutConv(base_c * 1, n_classes * 2)

    def forward(self, x):
        hidden_state2 = cell_state2 = torch.zeros(self.base_c, x.size(2), x.size(3)).to(device)
        convlstm2_outputs = []
        for t in range(x.size(0)):
            hidden_state2, cell_state2 = self.convlstm(x[t], [hidden_state2, cell_state2])
            convlstm2_outputs.append(hidden_state2)
        x1 = torch.stack(convlstm2_outputs, dim=0)
        x2 = self.down1(x1)
        x3 = self.down2(x2)
        x4 = self.down3(x3)
        x5 = self.down4(x4)
